import Link from "next/link";

export default function BatchNotFound() {
  return (
    <div className="min-h-screen bg-[#020f12] flex flex-col items-center justify-center px-6 text-center">
      {/* Icon */}
      <div className="w-20 h-20 rounded-full bg-white/[0.04] border border-white/[0.08] flex items-center justify-center mb-6">
        <svg
          width="32"
          height="32"
          viewBox="0 0 24 24"
          fill="none"
          stroke="rgba(255,255,255,0.25)"
          strokeWidth="1.5"
        >
          <circle cx="11" cy="11" r="8" />
          <path d="M21 21l-4.35-4.35" />
          <line x1="11" y1="8" x2="11" y2="12" />
          <line x1="11" y1="15" x2="11.01" y2="15" />
        </svg>
      </div>

      {/* Heading */}
      <h1
        className="text-white font-black text-2xl mb-3 leading-tight"
        style={{ fontFamily: "'Georgia', serif" }}
      >
        Batch Not Found
      </h1>

      {/* Message */}
      <p className="text-white/35 text-sm font-light max-w-xs leading-relaxed mb-2">
        This QR code doesn&apos;t match any batch in our system.
      </p>
      <p className="text-white/20 text-xs mb-8 tracking-wide">
        Please check the code on your pack or contact us on WhatsApp.
      </p>

      {/* Trust line */}
      <p className="text-white/15 text-[0.6rem] uppercase tracking-[0.14em] mb-8">
        Every legitimate DAKH batch is traceable.
      </p>

      {/* Actions */}
      <div className="flex flex-col gap-3 w-full max-w-xs">
        <Link
          href="/"
          className="flex items-center justify-center gap-2 bg-cyan-500/10 border border-cyan-500/25 text-cyan-400 font-semibold text-sm py-3.5 rounded-xl transition hover:bg-cyan-500/18"
        >
          ← Back to Homepage
        </Link>
        <a
          href="https://wa.me/919999999999?text=Hi%20DAKH!%20I%20scanned%20a%20QR%20code%20but%20the%20batch%20was%20not%20found."
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 bg-[#25D366]/10 border border-[#25D366]/25 text-[#25D366] font-semibold text-sm py-3.5 rounded-xl transition hover:bg-[#25D366]/18"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
          Contact on WhatsApp
        </a>
      </div>

      {/* Brand footer */}
      <p className="text-white/10 text-[0.58rem] mt-10">
        DAKH Shrimps &amp; Co. · FSSAI Reg. 10013022000041
      </p>
    </div>
  );
}
